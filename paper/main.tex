...
\begin{tikzpicture}[
    ...
    block/.style = {rectangle, draw, fill=blue!10, text width=6.5em, text centered, rounded corners, minimum height=2.5em},
    database/.style = {cylinder, draw, fill=gray!10, shape border rotate=90, text width=5.5em, text centered, minimum height=2.5em}
]
    ...
    \path [draw, -{Latex}] (user) -- (camp);
    \path [draw, <->] (camp) -- (registry);
    \path [draw, -{Latex}] (camp) -- (worker);
    \path [draw, <->] (worker) -- (gpu);
    \path [draw, -{Latex}] (worker) -- (mcp);
    \path [draw, <->] (mcp) -- (crypto);
\end{tikzpicture}
